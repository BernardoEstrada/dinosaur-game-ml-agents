...
namespace UnityStandardAssets.ImageEffects {
    ...
    public class CameraMotionBlur : PostEffectsBase {
        ...
        private static readonly float MAX_RADIUS = 10.0f;
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination) {
            ...
            // get temp textures
            var velBuffer = RenderTexture.GetTemporary(divRoundUp(source.width, velocityDownsample),
                divRoundUp(source.height, velocityDownsample), 0, rtFormat);
            var tileWidth = 1;
            var tileHeight = 1;
            ...
            // just started up?
            if (gameObject.activeInHierarchy && !wasActive) Remember();
            wasActive = gameObject.activeInHierarchy;
            ...
            if (_camera.stereoEnabled) {
                var invStereoViewPrj = new Matrix4x4[2];
                ...
                var combined = prevStereoViewProjMat[0] * invStereoViewPrj[0];
                motionBlurMaterial.SetMatrix("_StereoToPrevViewProjCombined0", combined);
                motionBlurMaterial.SetMatrix("_StereoToPrevViewProjCombined1",
                    prevStereoViewProjMat[1] * invStereoViewPrj[1]);
            }
            ...
            if (filterType == MotionBlurFilter.CameraMotion) {
                ...
                farHeur = Vector3.Angle(transform.forward, prevFrameForward) / _camera.fieldOfView *
                          (source.width * 0.75f);
                blurVector.y =
                    rotationScale * lookUpDown *
                    farHeur; //Mathf.Clamp01((1.0ff-Vector3.Dot(transform.forward, prevFrameForward)));

                // yaw #2 (when looking down, faded by 1-pitch)
                farHeur = Vector3.Angle(transform.forward, prevFrameForward) / _camera.fieldOfView *
                          (source.width * 0.75f);
                blurVector.z =
                    rotationScale * (1.0f - lookUpDown) *
                    farHeur; //Mathf.Clamp01((1.0ff-Vector3.Dot(transform.forward, prevFrameForward)));

                if (distMag > Mathf.Epsilon && movementScale > Mathf.Epsilon) {
                    // forward (probably most important)
                    blurVector.w = movementScale * Vector3.Dot(transform.forward, distanceVector) *
                                   (source.width * 0.5f);
                    ...
                    blurVector.y += movementScale * Vector3.Dot(transform.right, distanceVector) *
                                    (source.width * 0.5f);
                }
        ...
        private void CalculateViewProjection() {
            ...
            if (_camera.stereoEnabled)
                for (var eye = 0; eye < 2; ++eye) {
                    var stereoViewMat =
                        _camera.GetStereoViewMatrix(eye == 0
                            ? Camera.StereoscopicEye.Left
                            : Camera.StereoscopicEye.Right);
                    var stereoProjMat =
                        _camera.GetStereoProjectionMatrix(eye == 0
                            ? Camera.StereoscopicEye.Left
                            : Camera.StereoscopicEye.Right);
