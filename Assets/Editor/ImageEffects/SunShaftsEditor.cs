...
namespace UnityStandardAssets.ImageEffects {
    ...
    internal class SunShaftsEditor : Editor {
		...
		*/
            ...
            if ((target as SunShafts).sunTransform && (target as SunShafts).GetComponent<Camera>())
                if (GUILayout.Button("Center on " + (target as SunShafts).GetComponent<Camera>().name))
                    if (EditorUtility.DisplayDialog("Move sun shafts source?",
                        ...
                        var ray = (target as SunShafts).GetComponent<Camera>()
            ...
            maxRadius.floatValue =
                1.0f - EditorGUILayout.Slider("Distance falloff", 1.0f - maxRadius.floatValue, 0.1f, 1.0f);

            EditorGUILayout.Separator();

            sunShaftBlurRadius.floatValue =
                EditorGUILayout.Slider("Blur size", sunShaftBlurRadius.floatValue, 1.0f, 10.0f);
            radialBlurIterations.intValue =
                EditorGUILayout.IntSlider("Blur iterations", radialBlurIterations.intValue, 1, 3);
