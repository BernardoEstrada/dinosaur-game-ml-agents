    ...
    internal class DepthOfFieldDeprecatedEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            if (simpleTweakMode.boolValue)
                GUILayout.Label(
                    "Current: " + go.GetComponent<Camera>().name + ", near " + go.GetComponent<Camera>().nearClipPlane +
                    ", far: " + go.GetComponent<Camera>().farClipPlane + ", focal: " + focalPoint.floatValue,
                    EditorStyles.miniBoldLabel);
            else
                GUILayout.Label(
                    "Current: " + go.GetComponent<Camera>().name + ", near " + go.GetComponent<Camera>().nearClipPlane +
                    ", far: " + go.GetComponent<Camera>().farClipPlane + ", focal: " + focalZDistance.floatValue,
                    EditorStyles.miniBoldLabel);
            ...
            if (simpleTweakMode.boolValue) {
                ...
                focalSize.floatValue = EditorGUILayout.Slider("Size", focalSize.floatValue, 0.0f,
                    go.GetComponent<Camera>().farClipPlane - go.GetComponent<Camera>().nearClipPlane);
                focalStartCurve.floatValue =
                    EditorGUILayout.Slider("Start curve", focalStartCurve.floatValue, 0.05f, 20.0f);
            ...
            if (bokeh.boolValue) {
                ...
                bokehThresholdLuminance.floatValue =
                    EditorGUILayout.Slider("Min luminance", bokehThresholdLuminance.floatValue, 0.0f, 0.99f);
                bokehThresholdContrast.floatValue =
                    EditorGUILayout.Slider("Min contrast", bokehThresholdContrast.floatValue, 0.0f, 0.25f);
